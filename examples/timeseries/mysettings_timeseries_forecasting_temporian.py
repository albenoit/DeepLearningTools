...
serving_client_timeout_int_secs = (
    20  # timeout limit when a client requests a served model
)
...
jittered_repeats_per_epoch = 10
nb_train_samples = jittered_repeats_per_epoch*2000 // temporal_series_length
nb_val_samples = 2000 // temporal_series_length  # the 2 sample CSV files represent 2000 points
...
reference_labels = [
    "startDate",
    "stopDate",
]  # to be used if many labels are generated by the get_input_pipeline_train_val function
...
feature_names = [
    ...
    "RDC-ChambreEnfants_Humidité_HUMIDITY%<>Avg",
    "RDC-ChambreParents_Humidité_HUMIDITY%<>Avg",
    "RDC-Séjour_Humidité_HUMIDITY%<>Avg",
    "Technique-Meteo_Humidité_absolue_Abs_humidityg/m3<>Avg",
    "Technique-Meteo_Humidité_relative_Rel_humidity%<>Avg",
    "RDC-ChambreEnfants_Température_TEMPERATURE°C<>Avg",
    "RDC-ChambreParents_Température_TEMPERATURE°C<>Avg",
    "RDC-Séjour_Température_TEMPERATURE°C<>Avg",
    "prev_day_worked",
    "day_worked",
    "next_day_worked",
]
ts_feature_names = feature_names[2:-3]
print("######################## sensors feature names", ts_feature_names)
...
wait_for_server_ready_int_secs = 5
serving_client_timeout_int_secs = (
    1  # timeout limit when a client requests a served model
)
served_input_names = [
    "temporal_features",
    "yesterday_isfree",
...
def get_learningRate():
    ...
    Returns a scalar (float) or a scheduler
    """
...
def get_input_pipeline(raw_data_files_folder, isTraining, batch_size, nbEpoch):
    ...
    """
    from deeplearningtools import (  # only import here to reduce dependencies in serving mode
        DataProvider_input_pipeline,
    )

    # load all csv files to use for training
    raw_data_files = DataProvider_input_pipeline.extractFilenames(
        root_dir=raw_data_files_folder, file_extension=raw_data_filename_extension
    )
    # sort files in numeric order wrt the last integer before file extension
    raw_data_files = sorted(
        raw_data_files, key=lambda e: int((e.split(".")[-2]).split("_")[-1])
    )
    print("Input files found (SORTED IN TIME)=" + str(raw_data_files))

    """ REMINDER: columns available in this demo:
        ['enumerate', 'lastDate', 'RDC-ChambreEnfants_CO2_GAS_CONCENTRATIONppm<>Avg', 'RDC-ChambreParents_CO2_GAS_CONCENTRATIONppm<>Avg', 'RDC-Séjour_CO2_GAS_CONCENTRATIONppm<>Avg', 'RDC-ChambreEnfants_Humidité_HUMIDITY%<>Avg', 'RDC-ChambreParents_Humidité_HUMIDITY%<>Avg', 'RDC-Séjour_Humidité_HUMIDITY%<>Avg', 'Technique-Meteo_Humidité_absolue_Abs_humidityg/m3<>Avg', 'Technique-Meteo_Humidité_relative_Rel_humidity%<>Avg', 'RDC-ChambreEnfants_Température_TEMPERATURE°C<>Avg', 'RDC-ChambreParents_Température_TEMPERATURE°C<>Avg', 'RDC-Séjour_Température_TEMPERATURE°C<>Avg', 'prev_day_worked', 'day_worked', 'next_day_worked']
    """

    # define here a custom function to preprocess each sample
    @tf.function
    def per_sample_process_function(single_period_data_block_raw):
        ...
        # DEBUG PURPOSE, UNCOMMENT: tf.print('single_period_data_block_raw', single_period_data_block_raw)

        # select each feature listed in ts_feature_names from the input dict:
        ts_cols = [
            tf.cast(
                tf.convert_to_tensor(single_period_data_block_raw[feature]), tf.float32
            )
            for feature in ts_feature_names
        ]
        single_period_data_block_ts = tf.stack(ts_cols, axis=1)
        """tf.print('single_period_data_block_raw TS', single_period_data_block_raw)
        tf.print('prev_day_worked', single_period_data_block_raw['prev_day_worked'])
        tf.print('prev_day_worked[0]', single_period_data_block_raw['prev_day_worked'][0], tf.shape(single_period_data_block_raw['prev_day_worked'][0]))
        """
        yesterday_isfree = single_period_data_block_raw["prev_day_worked"][0]
        today_isfree = single_period_data_block_raw["day_worked"][0]
        tomorrow_isfree = single_period_data_block_raw["next_day_worked"][0]

        # normalizing data wrt mean and std measured on the train set
        _means = tf.constant(mean_vals, shape=[1, len(mean_vals)])
        _vars = tf.constant(std_vals, shape=[1, len(mean_vals)])
        ts_all = tf.divide(
            tf.math.subtract(single_period_data_block_ts, _means), _vars + 1e-6
        )
        print("ts_all", ts_all)
        # stacking labels

        stack = {
            "input_time_series": ts_all[: hparams["tsLengthIn"], :],
            "input_yesterday_isfree": yesterday_isfree,
            "input_today_isfree": today_isfree,
            "input_tomorrow_isfree": tomorrow_isfree,
        }, ts_all[hparams["tsLengthIn"] :, :]
        print("sample content", stack)
        print("### per_sample_process_function END")
        return stack

    from deeplearningtools.datasets.temporian_dataloader import build_timeseries_tfdataset

    # define a local filter function (optionnal, here we remove non finite values)
    def filter_is_finite(example):
        return tf.reduce_all(
            [tf.math.is_finite(example[key]) for key in ts_feature_names]
        )  # data_is_finite and label_is_finite

    """
    temporian_events = temporian_dataloader.temporian_load_csv(filename=raw_data_files[0],
                                                      timestamps_colname='firstDate',
                                                      sep=csv_field_delim,
                                                      selected_features=[])
    features = temporian_events.schema.feature_names()
    print('File columns:', features)            
    
    dataset=temporian_dataloader.build_single_temporian_tfdataset(temporian_data=temporian_events,
                                        temporal_series_length=temporal_series_length,
                                        batch_size=batch_size,
                                        prepare_example_fn=per_sample_process_function,
    ...
    for filename in raw_data_files:
        single_csv_dataset=build_timeseries_tfdataset(
            filename=filename,
            timestamps_colname="firstDate",
            sep=csv_field_delim,
            selected_features=[],
            temporal_series_length=temporal_series_length,
            batch_size=0,#we do not batch here to be able to shuffle the datasets within batches next
            prepare_example_fn=per_sample_process_function,
            filter_fn=filter_is_finite,
            shuffle_size=10 if isTraining else 0,
            jitter=isTraining,
        ).repeat(nbEpoch)
        
        datasets.append(single_csv_dataset)
        datasets_len.append(single_csv_dataset.cardinality())
    nb_collections = len(datasets)
    print('datasets_len', datasets_len, 'total', sum(datasets_len))
    print('nb_collections', nb_collections)

    # interleave datasets in random order
    choice_dataset = tf.data.Dataset.random(seed=random_seed, rerandomize_each_iteration=True).map(lambda x: tf.math.floormod(x, nb_collections))
    dataset = tf.data.Dataset.choose_from_datasets(datasets, choice_dataset, stop_on_empty_dataset=False).repeat(nbEpoch)

    print("dataset", dataset)
    #batching is applied in the end after the interleaving of the datasets
        ...
        @tf.function(
            input_signature=[
                tf.TensorSpec(
                    ...
                    dtype=tf.float32,
                    name=served_input_names[0],
                ),  #'temporal_features'),
                tf.TensorSpec(
                    shape=(None, 1), dtype=tf.float32, name=served_input_names[1]
                ),  #'yesterday_isfree'),
                tf.TensorSpec(
                    shape=(None, 1), dtype=tf.float32, name=served_input_names[2]
                ),  #'today_isfree'),
                tf.TensorSpec(
                    shape=(None, 1), dtype=tf.float32, name=served_input_names[3]
                ),
            ]
        )  #'tomorrow_isfree')])
...
class Client_IO:
    ...
    def __init__(self, clientInitSpecs={}, debugMode=False):
        ...
        filenames = glob.glob(
            os.path.join(os.getcwd(), "../../../../../", raw_data_dir_val, "*.csv")
        )
        ...
        self.inputdata = pd.read_csv(filenames[0], delimiter=csv_field_delim).values[
            :, 1:
        ]
        if debugMode is True:
        ...
        self.current_time_idx = (
            self.neighborhood_range * temporal_series_length + field_of_view
        )
        # prepare plots
        self.firstCall = True
        self.fig, (self.ax1) = plt.subplots(
            1, 1, sharex=True, sharey=True
        )  # add some more subplots if required
    ...
    def getInputData(self, idx):
        ...
        t_start = (
            self.current_time_idx - hparams["tsLengthIn"] * self.neighborhood_range
        )
        t_stop = (
            self.current_time_idx + hparams["tsLengthOut"] * self.neighborhood_range
        )
        ...
        present_start = (
            hparams["tsLengthIn"] * self.neighborhood_range
        )  # an offset from tt_start to reach the index of self.current_time_idx
        ...
        if self.debugMode is True:
            print(
                "Generating input features (random values) of shape "
                + str(self.target.shape)
            )
            print(
                "selecting temporal window between indexes:",
                (request_t_start, request_t_stop),
            )
        ...
        temporal_features_expected_answer = self.target[
            request_t_stop : request_t_stop + hparams["tsLengthOut"], :11
        ]
        ...
        self.request_values = (
            temporal_features_request  # .reshape(time_series_input_serving_shape[1:])
        )

        # prepare expected answer for demo/analysis
        self.answer_xticks = self.x[
            request_t_stop : request_t_stop + hparams["tsLengthOut"]
        ]
        ...
        self.expected_answer_values = (
            self.expected_answer_values - self.mean_vals
        ) / self.std_vals
        # add batch dimension
        self.request_values = np.expand_dims(self.request_values, 0)
        return {
            served_input_names[0]: self.request_values.astype(np.float32),
    ...
    def decodeResponse(self, result):
        ...
        """
        response_recons = np.reshape(
            np.array(result.outputs[served_head_names[0]].float_val),
            [1, hparams["tsLengthOut"], 11],
        ).astype(np.float32)
        ...
        def standardize_timeseries(features):
            ...
            return (features - np.mean(features, axis=0)) / (
                np.std(features, axis=0) + 1e-6
            )

        self.ax1.cla()
        # print('self.request_xticks', self.request_xticks.shape)
        # print('self.answer_xticks', self.answer_xticks.shape)
        timeseries_ticks = np.concatenate(
            [self.request_xticks, self.answer_xticks], axis=0
        )
        # print('timeseries_ticks', timeseries_ticks.shape)
        timeseries_values = np.concatenate(
            [self.request_values[0], self.expected_answer_values], axis=0
        )
        # print('timeseries_values', timeseries_values.shape)
        self.ax1.plot(
            timeseries_ticks, timeseries_values
        )  # standardize_timeseries(self.request_values[0,:])))
