...
def group_norm(features, groups, mean_close_to_zero=False, eps=1e-6):
    """ Group normalisation from https://arxiv.org/pdf/1803.08494.pdf
		Args:
			features: the input features tensor
                        mean_close_to_zero: set True if input features mean is close to zero so that sufficient statistics are computed for accurate precision
			G: the number of groups to consider
		Returns the features normalized by sub groups
		Raises if the number of input channels and number of groups are not consistent 
    """
    with tf.variable_scope('group_norm'):
	N,H,W,C = features.shape
        if groups > C:
            raise ValueError('Invalid groups %d for %d channels.' % (groups, C))
        if C % groups != 0:
            raise ValueError('%d channels is not commensurate with %d groups.' %(C, groups))
	#create learable variables as for batch norm used to scale and shift the outing feature values
        gamma=tf.Variable(tf.ones([C], dtype=tf.float32), name='GN_gamma', trainable=True)
	beta= tf.Variable(tf.zeros([C], dtype=tf.float32), name='GN_gamma', trainable=True)
	tf.summary.histogram('GN_gamma', gamma)
        tf.summary.histogram('GN_beta', beta)

        """create a n+1 dimension tensor representation of the
	input features to separate channels into G groups
	this resghape may generate errors if C//G is not a positive integer and if G*(C//G)!=C
	"""
	features_=tf.reshape(features, [N, H, W, C//groups, groups])
	#normalize each group independantly
	if mean_close_to_zero is False:
          mean, variance=tf.nn.moments(features_, [1,2,3], keep_dims=True)
	else:
          # One pass algorithm returns better result when mean is close to zero.
          counts, means_ss, variance_ss, _ = nn.sufficient_statistics(features_, [1,2,3], keep_dims=True)
          mean, variance = nn.normalize_moments(counts, means_ss, variance_ss, shift=None)
        # Compute normalization : faster code than (features_-mean)/tf.sqrt(variance+eps)*gamma+beta
        gain = tf.rsqrt(variance + eps)#=1/sqrt(variance+eps)
        features_normalized_ = features_*gain -mean*gain
	#then, get back to the initial shape
	features_normalized=tf.reshape(features_normalized_, [N,H,W,C])
	#return a globally shifted, scaled output tensor
	return features_normalized*gamma+beta
...
def composite_function(input_features, out_features, kernel_size, is_training, keep_prob, name):
    ...
    with tf.variable_scope('Composite_'+str(name)):
        preprocessed_features = tf.nn.relu(group_norm(input_features, groups=input_features.shape[-1]/16))#tf.layers.batch_normalization(input_features, training=is_training))
...
def transition_down(input_features, is_training, keep_prob, block_idx):
    ...
    with tf.variable_scope('TransitionDown_'+str(block_idx)):
        preprocessed_features = tf.nn.relu(group_norm(input_features, input_features.shape[-1]/16))#tf.layers.batch_normalization(input_features, training=is_training))
