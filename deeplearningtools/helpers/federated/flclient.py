...
class FlClient(fl.client.NumPyClient):
    ...
    def write_restart_config(self, monitored_loss_val):
        ''' writes a config file (after each client.fit call) to save last states
        enabling for warm restart with all necessary data, especially last best monitored value, epoch index, etc.
        in order to better track client model
        Args: monitored_loss_val: the last model monitored value, the one that triggers model checkpointing
        Returns nothing but a lastinfo_clientXXX.ini is writen in the current working directory
        '''
    ...
    def load_restart_config(self):
        ''' loads a config file (if exists) to recover last states
        enabling for warm restart with all necessary data, especially last best monitored value, epoch index, etc.
        in order to better track client model
        Args: none
        Returns nothing but updates some instance variables
        '''
        config_filename=self.get_client_config_filename()
